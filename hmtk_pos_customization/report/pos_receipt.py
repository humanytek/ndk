...
class order(report_sxw.rml_parse):
    ...
    def _cut_mx(self, vat):
        print 'cut_mx'
        if vat:
            print "vat", vat
            new_vat = vat.split('MX')[1]
            return new_vat
        else:
            return vat
    ...
    def _get_journal_amt(self, order_id):
        data={}
        sql = """ select abs.name as statement,aj.name,absl.amount as amt from account_bank_statement as abs
...
report_sxw.report_sxw('report.pos.receipt.new', 'pos.order', 'addons/hmtk_pos_customization/report/pos_receipt.rml', parser=order, header=False)
report_sxw.report_sxw('report.pos.receipt.espain', 'pos.order', 'addons/hmtk_pos_customization/report/pos_receipt_spainsh.rml', parser=order, header=False)
