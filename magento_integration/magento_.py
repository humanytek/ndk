...
class Instance(osv.Model):
    ...
    _columns = dict(
        ...
        url=fields.char('Magento Site URL', required=True, size=256),
        api_user=fields.char('API User', required=True, size=64),
        api_key=fields.char('API Password / Key', required=True, size=128),
        order_prefix=fields.char(
            'Sale Order Prefix', size=16, help=
...
class InstanceWebsite(osv.Model):
    ...
    _columns = dict(
        name=fields.char('Name', required=True, size=64),
        code=fields.char('Code', required=True, size=64, readonly=True,),
        ...
        last_updated_magento_to_openerp=fields.datetime(
            'Last Updated On',
        ),
        last_updated_openerp_to_magento=fields.datetime(
            'Last Updated On',
        ),
   )
    _defaults = dict(
    ...
    def export_inventory(self, cursor, user, ids=None, context=None):
        ...
        for website in self.browse(cursor, user, ids, context):
            self.export_inventory_to_magento(
                cursor, user, website, context, use_local_cr=False
            )

    def export_inventory_to_magento(
        self, cursor, user, website_id, context, use_local_cr=False
    ):
        """
        ...
        for magento_product in website.magento_products or []:
