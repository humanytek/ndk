...
class UpdateCatalog(osv.TransientModel):
    ...
    def update_catalog(self, cursor, user, ids, context):
        ...
        t = threading.Thread(target=self.update_products,
                             args = (cursor, user, website_id, context, True))
        t.daemon = True
        t.start()

        return True#self.open_products(cursor, user, ids, product_ids, context)
        
    def link_products_magento_openerp(self, cr, uid, ids, context=None):
        product_obj = self.pool.get('product.product')
        
        website_id = context.get('active_id')
        
        t = threading.Thread(target=product_obj.link_magento_and_openerp_products_using_default_code, 
                             args = (cr, uid, website_id, context, True))
        t.daemon = True
        t.start()
        return True
    
    def link_categories_magento_openerp(self, cr, uid, ids, context=None):
        category_obj = self.pool.get('product.category')
        
        website_id = context.get('active_id')
        t = threading.Thread(target=category_obj.link_magento_and_openerp_categories,
                             args = (cr, uid, website_id, context, True))
        t.daemon = True
        t.start()
        return True
...
#             local_cr = db.cursor()
        ...
        if website.last_updated_magento_to_openerp: #Filter on the basis of last updated details on magento
            ...
            magento_product_ids = prod_website_obj.search(local_cr, user, [('magento_id','in',magento_ids_to_update)], context=context)
...
#             local_cr.close()
    ...
    def update_products_to_magento(self, cr, uid, ids, context=None):
        product_obj = self.pool.get('product.product')
        website_id = context.get('active_id')
        
        t = threading.Thread(target=product_obj.update_products_to_magento,
                             args=(cr, uid, website_id, context, True))
        t.daemon = True
        t.start()
        return True
